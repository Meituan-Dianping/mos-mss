 ...
 *
 * @!attribute abort
 *   Return this value = require(an iterator function {each} or {arrayEach}
...
let util = {
    ...
    jamespath: {
        query: function query(expression, data) {
            ...
            let results = [];
            let expressions = expression.split(/\s+or\s+/);
   ...
   */
    ...
    crypto: {
        ...
        crc32: function crc32(data) {
            let tbl = util.crypto.crc32Table;
            let crc = 0 ^ -1;
   ...
   */
    inherit: function inherit(klass, features) {
        ...
        if (features === undefined) {
            ...
            let ctor = function ConstructorWrapper() {};
   ...
   */
    mixin: function mixin() {
        let klass = arguments[0];
        for (let i = 1; i < arguments.length; i++) {
            for (let prop in arguments[i].prototype) {
                let fn = arguments[i].prototype[prop];
                if (prop !== 'constructor') {
